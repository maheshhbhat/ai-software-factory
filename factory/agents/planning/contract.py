...
def _scope_resolves(pattern: str, files: set[str]) -> bool:
    ...
    #
    # Planning does not otherwise try to predict whether a declared-new
    # path is actually creatable by Git/the filesystem (a `.git` segment,
    # case variants of one, an ancestor that is itself a tracked file,
    # and so on): that turned out to be an open-ended list, not a finite
    # deterministic check, and none of it is a governance/authorization
    # concern -- an uncreatable path simply fails at Delivery, and the
    # Merge Gate still only allows files actually on the authorized
    # scope list. See issue #706 for the specific cases considered and
    # deliberately left to Delivery/tests/Merge Gate instead of Planning.
    # The one exception is a "./"/"../"-prefixed pattern: match_path()
    # never special-cases that prefix (it splits on "/" and matches
    # segment-by-segment), so "./app.js" can't authorize anything under
    # our own scope-matching semantics regardless of whether the target
    # is new or existing -- rejecting it isn't predicting Git behavior,
    # it recognizes the pattern can never resolve to anything.
